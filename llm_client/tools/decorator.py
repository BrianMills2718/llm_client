...
def tool(
    ...
    *,
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        # Extract type hints for registry metadata.
        ...
        # For input type, use the first non-return annotation if there's
        # a single Pydantic-style input parameter
        param_hints = {k: v for k, v in hints.items() if k != "return"}
        if len(param_hints) == 1:
            input_type_hint = next(iter(param_hints.values()))
        ...
        info = ToolInfo(
            ...
            output_type=output_type_hint if isinstance(output_type_hint, type) else None,
        )
